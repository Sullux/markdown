...
test('markdownToHtml - Headings and slugified IDs', () => {
  const md = '# Main Title\n\n## Section One\n\n## Section One'
  const { html, head } = markdownToHtml(md)
  assert.strictEqual(
    html,
...
test('markdownToHtml - Pluggable custom code block renderers with head assets', () => {
  const md = '```custom\ncustom content\n```'
  const customRenderer = (node) => ({
    html: `<div class="custom-block">${node.value}</div>\n`,
    head: ['<script src="custom.js"></script>'],
  })

  const { html, head } = markdownToHtml(md, {
    codeRenderers: { custom: customRenderer },
  })

  assert.strictEqual(html, '<div class="custom-block">custom content</div>\n')
  assert.deepStrictEqual(head, ['<script src="custom.js"></script>'])
})

test('markdownToHtml - GitHub and GitBook callout boxes', () => {
  const gfmMd = '> [!NOTE]\n> This is a note.'
  const { html: gfmHtml } = markdownToHtml(gfmMd)
  assert.match(gfmHtml, /<div class="callout callout-note">\n<div class="callout-title">Note<\/div>\n<p>This is a note.<\/p>\n<\/div>/)

  const gitbookMd = '{% hint style="info" %}\nThis is info.\n{% endhint %}'
  const { html: gitbookHtml } = markdownToHtml(gitbookMd)
...
test('markdownToHtml - Renders inline math and display math with semantic container defaults and KaTeX head', () => {
  ...
  assert.ok(head.some((h) => h.includes('katex.min.css')))
  assert.ok(head.some((h) => h.includes('auto-render.min.js')))
})
...
test('markdownToHtml - Default codeBlock with math language renders as math-display with KaTeX head', () => {
  const md = '```math\nf(x) = x^2\n```'
  const { html, head } = markdownToHtml(md)
  assert.ok(html.includes('<div class="math-display" data-latex="f(x) = x^2">$$\nf(x) = x^2\n$$</div>'))
  assert.ok(head.some((h) => h.includes('katex.min.css')))
})

test('markdownToHtml - Default codeBlock with mermaid language renders as pre.mermaid with Mermaid head and theme listener', () => {
  const md = '```mermaid\ngraph TD;\n  A-->B;\n```'
  const { html, head } = markdownToHtml(md)
  assert.ok(html.includes('<pre class="mermaid">graph TD;\n  A--&gt;B;</pre>'))
  assert.ok(head.some((h) => h.includes('mermaid') && h.includes('@sullux/markdown:theme')))
})
